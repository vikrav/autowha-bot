...
async function handleAutoReply(from, text) {
  try {
    ...
    console.log('RAW FROM:', from); const phone = from.replace(/@s\.whatsapp\.net|@lid/g, '').replace(/\D/g, '');
...
setInterval(async()=>{try{await fetch(`${SERVER}/api/whatsapp/heartbeat`,{method:'POST',headers:{Authorization:`Bearer ${TOKEN}`}});}catch(e){}},30000);
    }
    if (connection === 'close') {
